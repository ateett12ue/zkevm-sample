...
import { ZeroHash, hexlify, id, toBeArray, toBeHex, toBigInt } from "ethers";
...
const ZkTree: NextPage = () => {
      ...
      await writeContractAsync(
        {
          ...
          args: [`0x${proofForContract.slice(2)}`, BigInt(indexForContract), `0x${nullifierHashForContract.slice(2)}`],
        },
        {
          onBlockConfirmation: txnReceipt => {
            console.log("📦 Transaction blockHash", txnReceipt.blockHash);
  ...
  const getNullifierHash = () => {
    setNullierHash(
      toBeHex(poseidon4([Number(indexForNullifier), hashedSecretForNullifier, callerForNullifier, leafForNullifier])),
    );
  };
  ...
  const shorten = (input: string) => {
    ...
    return (
      <p className="flex items-center text-lg mt-1 ml-4">
        {temp}
        {isCopied ? (
          <CheckCircleIcon
          ...
          >
            <DocumentDuplicateIcon className="text-xl font-normal text-white-600 h-5 w-5 cursor-pointer mx-2" />
          </CopyToClipboard>
        )}
      </p>
    );
  ...
  useEffect(() => {
    ...
    indexForProof,
    hashedSecretForProof,
    callerForProof,
    leafForProof,
    rootForProof,
    merklePathArray,
    nullifierHashForProof,
  ]);
    ...
    <>
      <div className="flex items-center flex-col flex-grow pt-10">
        <div className="px-5">
          ...
          <p className="text-center text-lg">This extension implements Zero Knowledge Merkle Tree.</p>
        ...
        <div className="flex items-center flex-col flex-grow rounded-3xl shadow-xl border-2 px-5">
          <p className="text-center text-lg">Get your hashed secret:</p>
          <input
            type="text"
        ...
        <div className="flex items-center flex-col flex-grow rounded-3xl shadow-xl border-2 px-5 mt-5">
          <p className="text-center text-lg">Get a new leaf from a hashed secret:</p>
          <Bytes32Input
        ...
        <div className="flex items-center flex-col flex-grow rounded-3xl shadow-xl border-2 px-5 mt-5">
          <p className="text-center text-lg">Add a new leaf to the tree:</p>
          <Bytes32Input
            name="leafForTree"
            value={leafForTree}
            placeholder="Leaf"
          ...
          {index != -1 && <p className="text-center text-lg -mt-2">Your index: {index}</p>}
          {merklePath[0] != "" && (
            <p className="text-center text-lg -mt-2">
              Your merkle path:
        ...
        <div className="flex items-center flex-col flex-grow rounded-3xl shadow-xl border-2 px-5 mt-5">
          <p className="text-center text-lg">Remove a leaf from the tree:</p>
          <IntegerInput
            name="leafToRemove"
            value={leafToRemove}
            placeholder="Index"
        ...
        <div className="flex items-center flex-col flex-grow rounded-3xl shadow-xl border-2 px-5 mt-5 py-3">
          <p className="text-center text-lg">Get the root of the tree at:</p>
          <IntegerInput
            name="indexForRoot"
            value={indexForRoot}
            placeholder="Index"
          ...
          />
          {indexForRoot != "" && RetrievedRoot == ZeroHash && <p className="text-center text-lg">No Root!</p>}
        ...
        <div className="flex items-center flex-col flex-grow rounded-3xl shadow-xl border-2 px-5 mt-5">
          <p className="text-center text-lg">Get the nullifier hash:</p>
          <div className="mt-1">
            <IntegerInput
        ...
        <div className="flex items-center flex-col flex-grow rounded-3xl shadow-xl border-2 px-5 mt-5">
          <p className="text-center text-lg">Create proof (takes few minutes!):</p>
          <div className="mt-1">
            <input
              type="text"
        ...
        <div className="flex items-center flex-col flex-grow rounded-3xl shadow-xl border-2 px-5 mt-5">
          <p className="text-center text-lg">Verify proof (off-chain):</p>
          <div className="mt-1">
            <BytesInput
              name="proofToVerify"
              value={proofToVerify}
          ...
          {isVerified && (
            <p className="text-center text-lg -mt-2">{isValid ? "Proof is valid!" : "Proof is invalid!"}</p>
          )}
        </div>
        <div className="flex items-center flex-col flex-grow rounded-3xl shadow-xl border-2 px-5 mt-5">
          <p className="text-center text-lg">Call the example function (on-chain):</p>
          <div className="mt-1">
            <IntegerInput
          ...
          {isValidForContract && <p className="text-center text-lg -mt-2">Proof is valid!</p>}
        </div>
      </div>
    </>
  );
